# coding=utf-8
"""Finetuning the library models for question-answering on Dureader2020 (DistilBERT, Bert, XLM, XLNet)."""
...
# from model.modeling_bert import BertForQuestionAnswering, BertConfig
from helper import SquadResult, MyProcessor, squad_convert_examples_to_features_orig
from optimizer import Adam_GC, AdamW_GC

from transformers.models.bert.modeling_bert import BertForQuestionAnswering, BertConfig
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # TODO
            # 补全代码 2
            # 补全下面代码，根据上下文中的内容，完成模型训练部分
            # 中间对一些特殊模型的输入做了一些处理，这部分并没有删除掉
            # 在这部分后面继续补充训练的代码
            # ----------------------------------------------------------------------------------------
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and logging_steps > 0 and global_step % logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        # 按照最长公共子序列计算
                        res_baidu = baidu_evaluate(f'{args.data_dir}/{args.predict_file}',
                                                   f'{args.output_dir}/predictions_dev_{global_step}.json')
                        print(res_baidu)
                        ...
                        for key, value in res_baidu.items():
                            tb_writer.add_scalar("baidu_eval_{}".format(key), value, global_step)
...
def merge(args, tokenizer, prefix="test"):
    dataset, examples, features = load_and_cache_examples(args, tokenizer, set_type=prefix, output_examples=True)

    with open(f'{args.output_dir}/test_prob_k0.pkl', 'rb') as f:
        all_results_kf0 = pickle.load(f)
    with open(f'{args.output_dir}/test_prob_k1.pkl', 'rb') as f:
        all_results_kf1 = pickle.load(f)
    with open(f'{args.output_dir}/test_prob_k2.pkl', 'rb') as f:
        all_results_kf2 = pickle.load(f)
    with open(f'{args.output_dir}/test_prob_k3.pkl', 'rb') as f:
        all_results_kf3 = pickle.load(f)
    with open(f'{args.output_dir}/test_prob_k4.pkl', 'rb') as f:
        all_results_kf4 = pickle.load(f)

    all_results = []
    for r0, r1, r2, r3, r4 in zip(all_results_kf0, all_results_kf1, all_results_kf2, all_results_kf3, all_results_kf4):
        # print(r0.start_logits)
        unique_id = r0.unique_id
        start_logits = np.mean([r0.start_logits, r1.start_logits, r2.start_logits, r3.start_logits, r4.start_logits],
                               axis=0)
        end_logits = np.mean([r0.end_logits, r1.end_logits, r2.end_logits, r3.end_logits, r4.end_logits], axis=0)
        result = SquadResult(unique_id, start_logits, end_logits)
        all_results.append(result)

    # Compute predictions
    output_prediction_file = f"{args.output_dir}/merge_predictions_{prefix}.json"
    output_nbest_file = f"{args.output_dir}/merge_nbest_predictions_{prefix}.json"

    if args.version_2_with_negative:
        output_null_log_odds_file = f"{args.output_dir}/merge_null_odds_{prefix}.json"
    else:
        output_null_log_odds_file = None
    ...
    with open(f'{args.output_dir}/merge_test_prob.pkl', 'wb') as f:
        pickle.dump(all_results, f)
...
def main():
    ...
    # TODO
    # 补全代码 1
    # 补全下面代码，定义 PTMs 模型的 config、tokenizer、model
    # 需要和 args 传递进来的参数进行结合
    #   config: 与 args.config_name 和 args.model_name_or_path 相关
    #   tokenizer: 与 args.tokenizer_name 和 args.model_name_or_path 相关
    #   model: 与 args.model_name_or_path 和 args.cache_dir 相关
    # 尽可能的做出满足各种加载条件的模型
    # ----------------------------------------------------------------------------------------
